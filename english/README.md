...
- If the emulation with atari800 is so slow, that it doesn't work, the option `VIDEO_ACCEL=0` should be checked in the configuration file. `0` is the value that should work. So if there's anything else, it should be changed to that. I thought, another value would work with the proprietary graphics driver (Nvidia), but it didn't.
